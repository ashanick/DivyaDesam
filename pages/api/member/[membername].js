...
export default function handler({query:{membername}}, res) {
    console.log(' 🤳🤳🤳User Name Api 888888 =====: ', membername)
    var familyTreeList = []
    var userNodes = []
    var userLinks = []
    var memberDetail = []
    ...
    session
    .run(`OPTIONAL MATCH pathmem = (b:Member {name: '${membername}'}) - [r:MEMORY_OWN] ->(o:Memories) 
        OPTIONAL MATCH pathphoto = (a:Member {name: '${membername}'}) - [w:PHOTO_OWN] ->(q:PhotoGallery)
        OPTIONAL MATCH (m:Member {name: '${membername}'}) 
        OPTIONAL MATCH (j:Details {name: '${membername}'})
        OPTIONAL MATCH pathspouse =(ps:Member {name: '${membername}'})<-[pr:SPOUSE_OF]-(qs:Member)
        OPTIONAL MATCH path=(n:Member {name: '${membername}'})-[:PARENT_OF*1..5]-(p) 
